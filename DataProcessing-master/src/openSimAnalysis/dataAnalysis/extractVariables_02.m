		...
		if any(strcmp(fieldnames(ID_metrics_ordered.(subjects{sName})), conditionName))
			...
			for k = 1:length(metricsID)
				...
				switch variableName
					...
					case 'hip_flexion_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.hip_flexion_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
						...
						if notTBAS == 1
							...
							ID_metrics_for_analysis.([variableName, '_diffTBAS']).(conditionName)(:, sName) =...
								ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end)...
								- ID_metrics_ordered.(subjects{sName}).(tbasName).(variableName).mean(4:end);
						end
						
					case 'hip_adduction_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.hip_adduction_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
					...
					case 'hip_rotation_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.hip_rotation_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
					...
					case 'knee_angle_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.knee_flexion_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
						...
						ID_metrics_for_analysis.knee_flex_peak.(conditionName)(sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).max_min_range(1);
					...
					case 'knee_adduction_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.knee_adduction_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
					...
					case 'knee_rotation_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.knee_rotation_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
					...
					case 'ankle_angle_r_moment'
						ID_metrics_for_analysis.(variableName).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						ID_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						ID_metrics_for_analysis.ankle_flexion_power.(conditionName)(:, sName) = ID_metrics_ordered.(subjects{sName}).(conditionName).(variableName).JOINT_POWER(4:end);
			...
			for k = 1:length(metricsIK)
				...
				switch variableName
					...
					% conditions (z direction)
					case 'hip_flexion_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.hip_flexion_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
						...
						if notTBAS == 1
							...
							IK_metrics_for_analysis.([variableName, '_diffTBAS']).(conditionName)(:, sName) =...
								IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end)...
								- IK_metrics_ordered.(subjects{sName}).(tbasName).(variableName).mean(4:end);
						end
						
					case 'hip_adduction_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.hip_adduction_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
					...
					case 'hip_rotation_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.hip_rotation_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
					...
					case 'knee_angle_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.knee_flexion_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
						...
						IK_metrics_for_analysis.knee_flex_peak.(conditionName)(sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).max_min_range(1);
					...
					case 'knee_adduction_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.knee_adduction_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
					...
					case 'knee_rotation_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.knee_rotation_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
					...
					case 'ankle_angle_r'
						IK_metrics_for_analysis.(variableName).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).mean(4:end);
						IK_metrics_for_analysis.([variableName, '_SD']).(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).sd(4:end);
						IK_metrics_for_analysis.ankle_flexion_vel.(conditionName)(:, sName) = IK_metrics_ordered.(subjects{sName}).(conditionName).(variableName).ANGULAR_VEL;
