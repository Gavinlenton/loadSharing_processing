function [anglesJointMean] = findMeanOfROMTrials(anglesJoint, sessionConditions, matFileDir)
...
for t = 1:length(sessionConditions)
     ...
     for i = 1:length(trials)
          ...
          if length(idx) == 3
               ...
               anglesJointMean.(sessionConditions{t}).(trials{i}) = mean([trial1, trial2, trial3]);
          ...
          elseif length(idx) == 2
               ...
               anglesJointMean.(sessionConditions{t}).(trials{i}) = mean([trial1, trial2]);
               
          elseif length(idx) == 1
               trial1 = anglesJoint.(names{idx(1)}).(angle).Range;
               
               anglesJointMean.(sessionConditions{t}).(trials{i}) = trial1;
          else
